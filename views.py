...
def showCategories():
	...
	for item in db_items:
		category_name = str(session.query(Category.name).filter_by(id = item.category_id).one())
		items.append({'name': item.name, 'id': item.id, 'category_id': item.category_id, 'category_name': category_name[3:len(category_name)-3]})
...
def addItem():
	...
	if request.method == 'GET':
		...
		new = Item(name = request.form['name'], description = request.form['description'], category_id = request.form['category'])
		session.add(new)
		session.commit()
		return redirect(url_for('showCategories'))
